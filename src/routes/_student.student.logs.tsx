  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Add what you worked on today. Your faculty mentor sees this in real time.</p>
      </div>
        ...
        <CardContent className="p-5">
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <Textarea id="t" rows={4} value={transcript} onChange={(e) => setTranscript(e.target.value)} required maxLength={2000} />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="tags">Tech used (comma-separated)</Label>
                <Input id="tags" value={techTags} onChange={(e) => setTechTags(e.target.value)} placeholder="SQL, Python" />
              </div>
              <div>
                <Label htmlFor="s">Fruitfulness (1-5)</Label>
                <Input id="s" type="number" min={1} max={5} step={0.1} value={sentiment} onChange={(e) => setSentiment(e.target.value)} />
              </div>
            </div>
      ...
      <div className="space-y-3">
        <div className="text-xs uppercase tracking-widest text-muted-foreground">History · {logs.length} entries</div>
            ...
            <CardContent className="p-4">
              ...
              {l.tech_tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {l.tech_tags.map((t) => (<Badge key={t} variant="outline" className="text-[10px]">{t}</Badge>))}
                </div>
              )}
            </CardContent>
          </Card>
