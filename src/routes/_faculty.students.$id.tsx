...
type Submission = Tables<"internship_submissions">;
...
function StudentDetail() {
  ...
  const load = useCallback(async () => {
    ...
    if (s?.jd_file_path) {
      const { data: signed } = await supabase.storage
        .from("jd-files")
        .createSignedUrl(s.jd_file_path, 600);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {sub ? (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {sub.required_tech.map((t: string) => (
        ...
        <Card className="card-elevated border-border/60">
          <CardContent className="p-5 text-sm text-muted-foreground">
            Student has not submitted internship details yet.
          </CardContent>
        </Card>
      )}
      ...
      <Card className="card-elevated border-border/60">
        <CardHeader>
          <CardTitle className="font-display text-base">Daily Logs · {logs.length}</CardTitle>
        </CardHeader>
      ...
      <Card className="card-elevated border-border/60">
        <CardHeader>
          <CardTitle className="font-display text-base">Mentor Feedback Thread</CardTitle>
        </CardHeader>
