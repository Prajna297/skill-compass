  ...
  useEffect(() => {
    (async () => {
      ...
      if (ids.length) {
        ...
        (profs ?? []).forEach((p: any) => (map[p.id] = p.full_name || "Faculty"));
        setAuthors(map);
      }
    })();
  }, [user?.id]);
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Comments from your faculty mentor appear here.</p>
      </div>
        ...
        <Card key={c.id} className="card-elevated border-border/60">
          <CardContent className="p-5">
            <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
              <span className="font-medium text-foreground">{authors[c.faculty_id] || "Faculty"}</span>
