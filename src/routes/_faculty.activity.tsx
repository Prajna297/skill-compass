    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Real-time student logs · transcribed by Whisper, summarized by Falcon-7B.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {activityFeed.map((log) => (
          <Card
            key={log.id}
            className="card-elevated border-border/60 hover:border-primary/30 transition-colors"
          >
            <CardContent className="p-5">
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                ...
                {log.techTags.map((tag) => (
                  <Badge key={tag} variant="outline" className="border-info/40 text-info bg-info/5">
                    {tag}
                  </Badge>
                ))}
              </div>
